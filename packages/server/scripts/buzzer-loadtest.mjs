...
async function main() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    bots.push({ sock: c, id: joined.participant.id })
  }
  ...
  host.on(EVENTS.BUZZ_QUESTION_ENDED, async () => {
    await sleep(STEP_MS)
  ...
  const ok = locks === races && badWinner === 0 && errors === 0
  console.log(ok ? '✅ OK' : '❌ ANOMALIE détectée')
